...
func runTool(vaultsDataFile []ui.VaultsDataFile, vaultID string, nonceOverride int, nonceOverrideSet bool, requestIDOverride string, quorumOverride int, exportKSFile, passwordForKS string, privateKeysPEM [][]byte) (
	...
	if err != nil {
		...
		case errors.Is(err, recoverypipeline.ErrAmbiguousRootRequestID):
			welp = fmt.Errorf("%s; specify a request ID to disambiguate", err)
		case errors.Is(err, recoverypipeline.ErrRequestIDMismatch):
			welp = fmt.Errorf("%s; specify a request ID and threshold to disambiguate", err)
		default:
			welp = fileutils.StripPathFromError(err)
		}
		return
...
func getTSSPubKeyForEthereum(x, y *big.Int) (*secp256k1.PublicKey, string, error) {
	...
	// Coordinates MUST be zero-padded to 32 bytes: big.Int.Bytes() strips leading zero bytes,
	// which would produce an under-length key that ParsePubKey rejects.
	...
	sum := hash.Sum(nil)
	addr := fmt.Sprintf("0x%s", common.Bytes2Hex(sum[len(sum)-20:]))
