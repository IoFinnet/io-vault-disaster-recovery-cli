...
func TestTool_New_V2_List(t *testing.T) {
	...
	address, ecSK, edSK, vaultFormData, _, err := runTool(files, nil, nil, nil, nil, nil, nil, nil)
...
func TestTool_New_V2_List_ReportsReshareNonce(t *testing.T) {
	...
	_, _, _, vaults, _, err := runTool([]ui.VaultsDataFile{bvn, x2q, u44}, nil, nil, nil, nil, nil, nil, nil)
	...
	_, _, _, reversed, _, err := runTool([]ui.VaultsDataFile{u44, x2q, bvn}, nil, nil, nil, nil, nil, nil, nil)
	require.NoError(t, err)
...
func TestTool_New_V2_Export_lqns(t *testing.T) {
	...
	address, ecSK, edSK, vaultsFormData, _, err := runTool(files, &vaultID, nil, nil, nil, nil, nil, nil)
	if !assert.NoError(t, err) {
		return
	}
...
func TestTool_NewSingle_V2_List(t *testing.T) {
	...
	// use the correct file path for tests
	address, _, edSK, vaultFormData, _, err := runTool(files, nil, nil, nil, nil, nil, nil, nil)
	if !assert.NoError(t, err) {
		return
	}
...
func TestTool_NewSingle_V2_List_BadMnemonic(t *testing.T) {
	...
	// use the correct file path for tests
	_, _, _, _, _, err := runTool(files, nil, nil, nil, nil, nil, nil, nil)
...
func TestTool_NewSingle_V2_Export_qvl5(t *testing.T) {
	...
	_, ecSK, edSK, vaultsFormData, _, err := runTool(files, &vaultID, nil, nil, nil, nil, nil, nil)
	if !assert.NoError(t, err) {
		return
	}
	if !assert.Len(t, vaultsFormData, 1) {
...
func TestTool_Legacy_V2_List(t *testing.T) {
	...
	// use the correct file path for tests
	address, ecSK, edSK, vaultsFormData, _, err := runTool(files, nil, nil, nil, nil, nil, nil, nil)
	if !assert.NoError(t, err) {
		return
	}
	if !assert.Len(t, vaultsFormData, 1) {
...
func TestTool_Legacy_V1_IL_Export_m0k(t *testing.T) {
	...
	address, ecSK, edSK, vaultFormData, _, err := runTool(files, &vaultID, nil, nil, nil, nil, nil, nil)
...
func TestZipFileProcessing_V2_List(t *testing.T) {
	...
	address, ecSK, edSK, expectedVaultFormData, _, err := runTool(files, nil, nil, nil, nil, nil, nil, nil)
	require.NoError(t, err)
...
func TestZipFileProcessing_V2_Export_lqns(t *testing.T) {
	...
	expectedAddress, expectedEcSK, expectedEdSK, expectedVaultsFormData, _, err := runTool(files, &vaultID, nil, nil, nil, nil, nil, nil)
	require.NoError(t, err)
...
func TestHDAddressRecovery_EndToEnd(t *testing.T) {
	...
	_, ecSK, edSK, _, _, err := runTool(files, &vaultID, nil, nil, nil, nil, nil, nil)
	require.NoError(t, err)
